...
| `habits.html` | 私人习惯小记：目标、健身与睡眠打卡、最近七天睡眠条形图 |
| `post.html` | **未发布**的文章排版示例，已设置 `noindex` |

主导航的首页、代码人生、文章、关于我、好习惯分别跳转独立页面；项目总览的入口分别进入独立的技术记录页。旧的 `projects.html#data-pipeline`、`projects.html#law-design` 链接仍可定位到对应总览卡片；旧的 `index.html#projects` 链接会自动转到项目页。关于页可复制邮箱、分享页面链接（系统分享不可用时使用剪贴板）；不支持剪贴板时会明确提示。首页封面仅手动切换：箭头、圆点或手机左右划动；不会自动轮播。画风切换同时决定明暗主题：黑白原稿为暗色，彩色刊载为明亮；选择会在各页面记住。顶栏「本站数据」面板从实际项目卡与正式文章卡实时汇总内容图表及项目流程标签数，不采集访客，不伪造访问量；无法读取时显示错误而非零。搜索在站内已存在的项目记录、个人介绍及正式发布的文章卡片中查找（不把排版示例算作文章），`Ctrl/⌘+K` 聚焦、手机点放大镜展开；搜索需要 JavaScript，关闭时页面明确提示并提供直接入口。文章页的旧 `?s=` 查询和文章标签筛选仍可使用。文章详情提供阅读进度、目录高亮和代码复制。

## 好习惯小记（仅本机）

进入 `habits.html` 后可以记录目标（含可选目标日期、完成状态）、每日健身（时长及备注可选）和睡眠（入睡、起床日期时间）。睡眠支持跨午夜，按起床日期归档，同一天只记一条；图表显示最近七天的真实睡眠时长与入睡、起床时间，空白日期显示「未记录」。各类记录支持编辑、删除。

数据只保存在当前浏览器的 `localStorage` 中，**不上传 GitHub、不公开、不跨设备同步**。清除浏览器数据会使记录丢失；可通过页面底部「导出备份」保存 JSON，并在同一页面导入。导入会在确认后覆盖当前浏览器记录，请先导出原数据。私人记录不会进入站内搜索或「本站数据」统计。当前静态站点不支持直接连接小米手环；睡眠仍需手动填写，不会假装自动同步。
...
两个项目的真实 GitHub 仓库链接分别在项目卡与详情页，均明确标注为私有；项目页的六张界面图来自各自仓库的 `docs/screenshots/`，本站在 `assets/images/project/` 留有副本，图内数字仅为项目演示内容，不是本站实时数据；访客没有权限时仍可阅读公开的技术记录，或发邮件联系。项目内容要以实际代码和自己的更新为准，避免把规划中的链路写成已上线的成果。
