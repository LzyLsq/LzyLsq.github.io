...
- 漫画分镜与黑白／彩色配色：`css/style-comic.css`、`css/style-comic-upgrade.css` 和最后加载的 `css/manga-editions.css`。参考图只用于构图方向，没有作为网站素材；黑白版包含导航栏等全局界面均为灰阶，仅画风切换键保留与彩色版相同的四种主题色；彩色版切换键回到黑白；按键以放射／收束波纹切换当前页面，不刷新、不改变滚动位置（不支持页面过渡或选择减少动画时立即切换）。跨页仍使用真实链接，在支持的浏览器中接续画面并让标题与画格从两侧入场；首页翻页时画格和对白也从相反方向入场。切换逻辑在 `js/comic-mode.js`。明暗主题与漫画画风是同一个开关，由 `js/comic-mode.js` 一起切换。`css/style-memphis.css` 保留原有页面布局与兼容规则；修改时请同时检查黑白暗色／彩色明亮两种模式及桌面与手机对比度。
