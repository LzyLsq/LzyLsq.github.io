...
网站采用冷色调的动漫技术手账风格，保留少量便签与代码窗口元素，但**个人资料和项目文字独立撰写**。目前有两个项目介绍，**尚未正式发布文章**；文章页 `post.html` 只是排版示例，不计作已发布内容。
...
- 主配色与组件视觉：`css/style-notebook.css`，由最后加载的样式层统一控制明暗配色。`css/style-memphis.css` 保留原有页面布局与兼容规则；修改时请同时检查亮／暗主题、桌面与手机对比度。
