...
| 文件 | 内容 |
| --- | --- |
| `index.html` | Ryan 简介、两个项目、写作空状态、关于我入口 |
| `posts.html` | 文章列表／搜索；目前诚实显示空状态 |
| `about.html` | 个人简介、接触过的技术、邮箱与 GitHub |
| `post.html` | **未发布**的文章排版示例，已设置 `noindex` |

导航在页面间真实跳转；页内锚点即时定位。首页轮播支持箭头、圆点、手机左右划动，并在悬停、聚焦、页面隐藏或系统要求减少动效时停止自动轮播。明暗主题按系统偏好初始化并记住选择。搜索有无 JavaScript 的表单回退，`Ctrl/⌘+K` 聚焦、`Esc` 清空；正式文章发布后可按卡片内容和标签筛选。文章详情提供阅读进度、目录高亮和代码复制。
...
5. 若首页有新的轮播内容，记得在 `.hero-dots` 中添加对应 `data-hero-dot` 按钮；目前三张内容分别为个人简介、数据项目、AI 项目。
...
- 个人资料：`index.html`、`about.html`、`posts.html` 及各页页脚；联系方式还在导航的「更多」菜单及页眉邮件入口。
