...
- 公共样式：各页只请求一个生成的 `css/site-core.css`，以减少跨页重复加载的样式请求。修改 `css/` 下任一公共源文件后，运行 `python3 scripts/build-css.py` 并一同提交生成文件；`python3 scripts/build-css.py --check` 可验证是否同步。页面专用 CSS（如学习页、项目页）仍单独加载。此项减少请求数，不代表能消除所有浏览器／网络条件下的闪屏。
